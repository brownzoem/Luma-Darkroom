...
runtime and local inference runtime inspected on 2026-08-20. The URLs below are the canonical retrieval
...
| `ONNXRUNTIME-MIT.txt` | ONNX Runtime 1.27.0 license: <https://raw.githubusercontent.com/microsoft/onnxruntime/v1.27.0/LICENSE> |
| `ONNXRUNTIME-THIRD-PARTY-NOTICES.txt` | ONNX Runtime 1.27.0 third-party notice: <https://raw.githubusercontent.com/microsoft/onnxruntime/v1.27.0/ThirdPartyNotices.txt> |
...
The helper's `--refresh-onnxruntime` option refreshes only the two pinned ONNX
Runtime files and then rebuilds the checksum index. This is useful when other
upstream license hosts are temporarily unavailable; it does not skip the normal
release-time `--check-upstream` gate.
