...
(async()=>{
  ...
  const autoWhiteBalance=await page.evaluate(()=>({expected:analyzeNeutralWhiteBalance()}));
  await page.selectOption('[data-select-path="color.wb"]','Auto');
  Object.assign(autoWhiteBalance,await page.evaluate(()=>({mode:current.edits.color.wb,actual:[current.edits.color.temperature,current.edits.color.tint]})));
  if(autoWhiteBalance.mode!=='Auto'||autoWhiteBalance.actual.some((value,index)=>!Number.isFinite(value)||Math.abs(value-autoWhiteBalance.expected[index])>.11))errors.push(`Auto white balance did not apply its analysis: ${JSON.stringify(autoWhiteBalance)}`);
  ...
  await page.click('#batchPresetBtn');await page.waitForSelector('#presets:not(.hidden)');await page.click('.preset[data-name="Dynamic Range Look — Natural"]');
  ...
  // v3.0 keymap: in Develop, letters select tools (P = Pen); culling letters (P/X/U) flag from the Library view.
  const shortcutStart=await page.evaluate(()=>current.id);await page.keyboard.press('5');await page.keyboard.press('p');const shortcutDevelop=await page.evaluate(()=>({rating:current.rating,flag:current.flag,tool:toolMode}));await page.keyboard.press('Escape');await page.keyboard.press('ArrowRight');const shortcutEnd=await page.evaluate(()=>current.id);await page.click('.tab[data-view="library"]');const shortcutLibrary=await page.locator('body').evaluate(el=>el.classList.contains('library-mode'));await page.keyboard.press('p');const shortcutFlagged=await page.evaluate(()=>current.flag);await page.keyboard.press('d');const shortcutBack=await page.locator('body').evaluate(el=>!el.classList.contains('library-mode'));if(shortcutDevelop.rating!==5||shortcutDevelop.flag==='pick'||shortcutDevelop.tool!=='tool-pen'||shortcutEnd===shortcutStart||!shortcutLibrary||shortcutFlagged!=='pick'||!shortcutBack)errors.push('Culling keyboard shortcuts failed');
  ...
  const report={presetSwitch,controls,checks,autoWhiteBalance,selectState,curvePoints:curve.length,batch,compareState,shortcutDevelop,watermarks,overlap,security,errors};
